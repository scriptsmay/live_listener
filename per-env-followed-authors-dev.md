...
1. `config.js`：环境默认配置增加 `followedAuthors`，`getConfig()` 兼容旧全局关注列表。
2. `options.html`：移除全局关注主播输入区。
3. `options.js`：每个环境卡片独立渲染、读取和保存关注主播列表。
4. `background.js`：快手开播接口仍统一请求一次，请求结果按启用环境的关注列表过滤后定向推送。
5. `background.js`：`webRequest` 和 `content.js` 捕获链路也改为按页面标题筛选目标环境后推送。
...
`config.js` 中 `DEFAULT_ENVIRONMENTS` 已为每个环境增加默认 `followedAuthors` 字段：
...
`followedAuthors` 只作为旧调用方兼容字段保留，新代码使用 `environments[*].followedAuthors`。
...
`sendToBackend()` 保留为兼容包装，默认仍推送到所有启用环境。自动录制链路已经改为调用 `sendToEnvironments()`。

## 开播轮询匹配逻辑

`checkFollowingLivings()` 已改成环境级匹配：
...
`handleStreamerOnline()` 已增加 `targetEnvs` 参数，并将它传给推送函数。

## webRequest 与 content 捕获匹配逻辑

这两条链路没有快手关注接口返回的 `author.name`，只能根据页面标题匹配。当前已按环境分别判断：
...
`autoChooseBest()` 已接收并保留 `targetEnvs`，2 秒 debounce 后只推送到匹配环境。

## 状态查询与录制中跳过策略

当前 `activeRecordingRoomUrl` 是全局单值，并且轮询前只要任一环境仍在录制，就跳过快手开播列表请求。

环境级关注改造后，这里需要谨慎处理。首版保留当前策略，因为需求明确“关注列表的请求是统一发出的”，且现有逻辑本来就是全局跳过。但它有一个副作用：生产环境正在录制 A 时，开发环境关注的 B 开播可能不会被及时查询到。
...
1. 已修改 `config.js`：为环境增加默认 `followedAuthors`，调整 `getConfig()` 合并逻辑。
2. 已修改 `options.html`：删除全局关注区域，环境卡片复用 textarea 样式。
3. 已修改 `options.js`：渲染、读取、保存环境级主播列表。
4. 已修改 `background.js`：新增按目标环境推送函数，改造轮询、webRequest、content 三条自动录制链路。
5. 已做静态语法检查。仍需在 Chrome 中手动加载扩展验证真实快手接口和后端推送。
