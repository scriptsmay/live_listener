...
`handleStreamerOnline()` 优先从快手 API 返回的 `playUrls` 中选取最高画质的 FLV 地址，直接推送给匹配环境的后端。如果 API 未返回流地址，则尝试查找或打开该直播间页面，让 webRequest 链路捕获。同时创建 Chrome 桌面通知，点击可跳转到直播间。
...
右侧展示后台监听关注列表的开关状态、上次请求时间和结果。开关控制 `monitorEnabled`，切换时通知 Service Worker 重新检测。
